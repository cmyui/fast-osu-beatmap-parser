...
def differences(expected, actual, path=""):
    if isinstance(expected, dict):
        ...
        if (".path" in path or ".position" in path) and math.isclose(
            float(expected), float(actual), rel_tol=1e-6, abs_tol=1e-3
        ):
            return
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Debug a prefix; zero audits the whole corpus",
    )
    ...
    with subprocess.Popen(
        ...
        for path in paths:
            reference.stdin.write(
                json.dumps({"path": str(path.resolve()), "values": True}) + "\n"
            )
            reference.stdin.flush()
            expected = json.loads(
                reference.stdout.readline(),
            ...
            if not expected["ok"] or expected["projection_errors"]:
                ...
                gaps.append(
                    {
                        "file": str(path),
                        "reference_error": expected.get("error"),
            ...
            for obj, record in zip(beatmap.hit_objects, actual["hit_objects"]):
                if isinstance(obj, fosu.Slider) and obj.path is not None:
                    ...
                    record["path_samples"] = [
                        value(fosu.slider_position_at(obj.path, p))
            ...
            wanted = dict(
                raw["fields"],
                format_version=expected["format"],
                mode=expected["mode"],
                hit_objects=raw["hit_objects"],
                timing_points=raw["timing_points"],
                breaks=raw["breaks"],
                combo_colours=raw["combo_colours"],
            )
            wanted["stats"] = {
                "malformed_lines": len(expected["rejected"])
                + len(raw["policy_rejections"])
            }
