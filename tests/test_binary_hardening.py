...
def check(path):
    ...
    assert len(stacks) == 1 and 'E' not in ''.join(stacks[0][6:-1]), 'executable or unspecified stack'
    assert any(row[0] == 'GNU_RELRO' for row in segments), 'missing RELRO'
    assert re.search(r'\(BIND_NOW\)|\(FLAGS\).*BIND_NOW|\(FLAGS_1\).*\bNOW\b', elf), 'missing eager binding'
