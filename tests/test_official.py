...
STRICT_ENUM_CASES = {
    ...
    for field, values in (
        (
            "General.SampleSet",
            ("-1", "2147483647", "-2147483648", "  7  ", "\t7\t", "Normal, None"),
        ),
...
def fixtures():
    ...
    for section, keys in (
        ...
        (
            "General",
            "StackLeniency AudioLeadIn PreviewTime CountdownOffset Mode LetterboxInBreaks WidescreenStoryboard EpilepsyWarning SpecialStyle SamplesMatchPlaybackRate",
        ),
    ...
    for value in VALUES + ("131072", "131073", "-131073", "131072.001", "2147483776"):
        for index, name in enumerate(("x", "y", "time", "type", "hitsound")):
            ...
            fields[index] = value
            yield (
                f"object.{name}={value!r}",
                BASE + "[HitObjects]\n" + ",".join(fields) + "\n",
            )
        yield (
            f"slider.length={value!r}",
            BASE + "[HitObjects]\n256,192,1000,2,0,L|300:192,1," + value + "\n",
        )
        yield (
            f"slider.point={value!r}",
    ...
    for value in VALUES + (
        ...
        for head in (
            ...
            "256,192,1000,128,0,1500:",
        ):
            yield (
                f"sample={head + value!r}",
                BASE + "[HitObjects]\n" + head + value + "\n",
            )
        yield (
            f"slider.sample={value!r}",
            BASE + "[HitObjects]\n256,192,1000,2,0,L|300:192,1,100,,," + value + "\n",
        )
        yield (
            f"slider.edge_set={value!r}",
            BASE + "[HitObjects]\n256,192,1000,2,0,L|300:192,1,100,," + value + "\n",
        )
...
def main():
    ...
    parser.add_argument(
        "--reference", default="dotnet build/official-reference/OfficialReference.dll"
    )
    ...
    with subprocess.Popen(
        ...
        def check(name, path, data=None):
            ...
            if counts["files"] % 1000 == 0:
                print(
                    f"Checked {counts['files']} files; {len(gaps)} differences",
                    flush=True,
                )
    ...
    if args.report:
        args.report.write_text(
            json.dumps({"counts": dict(counts), "gaps": gaps}, indent=2) + "\n"
        )
