...
struct CApiView : fosu::BeatmapHeader {
    ...
    explicit CApiView(const fosu_view& v)
        : text(v.text), hit_objects(v.hit_objects, v.hit_object_count),
          sliders(v.sliders, v.slider_count), slider_points(v.points, v.point_count),
          timing_points(v.timing_points, v.timing_point_count), breaks(v.breaks, v.break_count),
          combo_colours(v.colours, v.colour_count),
          stats{v.stats.fast_path_lines, v.stats.slow_path_lines,
                v.stats.malformed_lines, v.stats.storyboard_lines} {
