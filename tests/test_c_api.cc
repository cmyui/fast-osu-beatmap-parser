...
void check(fosu_handle* h, const std::string& input, uint32_t sections = FOSU_ALL) {
  ...
  fosu::Parser parser;
  auto parsed = parser.parse(input.data(), input.size(), {.sections = sections});
  assert(parsed);
...
void check_growth() {
  ...
  for (int i = 0; i < 3000; ++i)
    many += std::to_string(i) + ",500\n";
  many += "[HitObjects]\n";
  for (int i = 0; i < 20000; ++i)
    many += "1,2,3,1,0\n";
  for (int i = 0; i < 5000; ++i)
    many += "1,2,3,2,0,B|1:2|3:4|5:6|7:8,1,10\n";
...
void check_invalid_input_clears_view() {
  ...
  assert(fosu_parse_file(handle, "/fosu-file-does-not-exist.osu", FOSU_ALL) ==
         FOSU_IO_ERROR);
  assert(errno == ENOENT);
  assert(!fosu_get_view(handle));
...
void check_file_matches_bytes() {
  ...
  assert(fd >= 0 &&
         write(fd, input.data(), input.size()) == static_cast<ssize_t>(input.size()));
  close(fd);
  auto* handle = fosu_new();
...
int main() {
  ...
  puts(
      "C API: exact values, defaults, reuse, lifetime, errors and independent handles "
      "passed");
}
