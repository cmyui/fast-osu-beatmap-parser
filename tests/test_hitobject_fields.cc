...
static void test_point_values() {
  ...
  for (const auto& test : cases) {
    ...
    const auto point =
        fosu::internal::parse_slider_point<Point>(input.data.get(), end, constants);
  ...
  for (const auto text :
       {"", "1:2", "|", "|1", "|1:", "|:2", "|bad:2", "|1:bad", "|131073:2"}) {
  ...
  CHECK(!fosu::internal::try_parse_slider_point_prefix_fast<Point>(input.data.get(),
                                                                   constants));
#endif
}
...
static void test_slider_fields() {
  ...
  for (const auto text :
    ...
    CHECK(!fosu::internal::parse_slider_tail(input.data.get(),
                                             input.data.get() + input.size, constants));
  }
