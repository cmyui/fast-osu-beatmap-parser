...
| Slider path | Compare raw curve choice, v128 segment types/degrees and control points with official coordinate conversion, before duplicate-point handling, relative coordinates, or perfect-curve normalization. Python's initial object-position point is included. Retained paths and samples come from the decoded official path. |
...
| Difficulty and editor fields | Compare official final precision, clamps, and defaults for difficulty, stack leniency, distance spacing, beat divisor, grid size, timeline zoom, preview time and slider-velocity presets. Float32 properties are widened to double before JSON serialization to preserve their exact value. |
