...
inline const char* libc_number(const char* p,
                               const char* end,
                               double&     out,
                               bool        nan) {
...
inline const char* libc_osu_float(const char* p,
  ...
  if (next == bounded.c_str() || !std::isfinite(out) || out < -limit ||
      out > limit)
    return p;
