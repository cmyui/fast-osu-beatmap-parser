...
inline const char* libc_number(const char* p, const char* end, double& out, bool nan) {
  ...
  return std::isfinite(out) || (nan && std::isnan(out)) ? p + (next - bounded.c_str())
                                                        : p;
}
...
inline const char* libc_beat_length(const char* p, const char* end, double& out) {
  ...
  return q != p && !(out < -INT32_MAX || out > INT32_MAX) ? skip_numeric_space(q, end)
                                                          : p;
}
...
inline const char* libc_osu_float(const char* p,
  ...
  if (next == bounded.c_str() || !std::isfinite(out) || out < -limit || out > limit)
    return p;
