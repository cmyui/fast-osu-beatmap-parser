#pragma once
#include <fosu/engine/document_sections.h>
...
#include <fosu/engine/timing_section.h>
...
// `data` must be followed by kBufferPadding readable zero bytes (io.h).
...
inline void parse_document(std::span<const char> input,
  ...
  while (p < file_end) {
    ...
    if (*p == '[') {
      ...
      if constexpr (UseSimd) {
        if (sec == Section::TimingPoints) {
          p = parse_timing_points_section(bm, timing_point_count, nl + (nl < file_end),
                                          file_end);
          sec = Section::Unknown;
          continue;
        }
        if (sec == Section::Events) {
          p = parse_events_section(bm, break_count, nl + (nl < file_end), file_end);
          sec = Section::Unknown;
          continue;
        }
      }
    ...
    if (fosu::internal::ignored_line(p, line_end))
      goto next_line;
    ...
    switch (sec) {
      ...
      case Section::General:
        if (len >= 5)
          parse_general_line(bm, p, len);
        break;
      case Section::Editor:
        if (len >= 5)
          parse_editor_line(bm, p, len);
        break;
      case Section::Metadata:
        if (len >= 5)
          parse_metadata_line(bm, p, len);
        break;
      case Section::Difficulty:
        if (len >= 5)
          parse_difficulty_line(bm, p, len, ar_specified);
        break;
      ...
      case Section::TimingPoints:
        parse_timing_point_line(bm, timing_point_count, p, len);
        break;
