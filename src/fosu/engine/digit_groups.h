...
inline DecimalChunks decode_decimal_chunks(
    const char* integer, uint32_t integer_digits,
    const char* fraction, uint32_t fraction_digits) {
    ...
    const auto fraction_mask = _mm_add_epi8(
        _mm_loadl_epi64(reinterpret_cast<const __m128i*>(
            kDigitAlignment[fraction_digits].data())), _mm_set1_epi8(8));
    const auto digits = _mm_shuffle_epi8(
        _mm_sub_epi8(text, _mm_set1_epi8('0')),
    ...
    const auto values = _mm_madd_epi16(
        _mm_packus_epi32(groups, groups),
