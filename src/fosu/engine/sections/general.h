...
inline std::optional<i32> parse_mode(std::string_view input) {
...
inline std::optional<i32> parse_legacy_enum(std::string_view            input,
                                            const StringLookup<i32, 4>& names) {
  ...
  do {
    ...
    while (part_end > p &&
           skip_numeric_space(part_end - 1, part_end) == part_end)
      --part_end;
    const auto* named_value =
        names.find({p, static_cast<size_t>(part_end - p)});
    if (!named_value)
      return std::nullopt;
    value |= *named_value;
    if (comma == end)
...
inline std::optional<i32> parse_countdown(std::string_view input) {
  static constexpr auto names = make_string_lookup<i32>({
      {"None", 0},
      {"Normal", 1},
      {"HalfSpeed", 2},
      {"DoubleSpeed", 3},
...
inline constexpr auto kGeneralFields = make_string_lookup<FieldParser>({
    ...
    {"Countdown",
     assign_field_value<&BeatmapHeader::countdown, parse_countdown>},
    {"SampleSet",
     assign_field_value<&BeatmapHeader::sample_set, parse_field_sample_set>},
    {"SampleVolume",
    ...
    {"WidescreenStoryboard",
     assign_field_value<&BeatmapHeader::widescreen_storyboard,
                        parse_field_boolean>},
    {"EpilepsyWarning",
