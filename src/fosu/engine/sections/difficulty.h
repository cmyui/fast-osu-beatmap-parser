...
inline std::optional<f64> parse_difficulty_rating(std::string_view input) {
  if (const auto value = parse_field_f32(input))
...
inline constexpr auto kDifficultyFields = make_string_lookup<FieldParser>({
    ...
    {"CircleSize", assign_field_value<&BeatmapHeader::cs, parse_field_f32>},
    {"OverallDifficulty",
