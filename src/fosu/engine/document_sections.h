...
#include <fosu/engine/metadata.h>
...
template <typename Map>
inline void parse_difficulty_line(Map& bm,
                                  const char* p,
                                  size_t len,
                                  bool& ar_specified) {
  ar_specified |=
      parse_kv_line<parse_double>(bm, kDifficulty, p, len, &bm.stats.malformed_lines);
}

inline void parse_colour_kv(Beatmap& bm,
                            size_t& colour_count,
                            std::string_view k,
                            std::string_view v) {
  if (k.substr(0, 5) != "Combo")
    return;
  const char* p = v.data();
