...
bool publish(Handle& handle, const fosu::Beatmap& map) {
    ...
    if (!hit_objects && !map.hit_objects.empty()) return allocation_failure();
    ...
    if (!sliders && !map.sliders.empty()) return allocation_failure();
    ...
    if (!points && !map.slider_points.empty()) return allocation_failure();
    ...
    if (!breaks && !map.breaks.empty()) return allocation_failure();
    ...
    if (!colours && !map.combo_colours.empty()) return allocation_failure();
    if (colours) {
        std::memcpy(
...
extern "C" int fosu_parse(fosu_handle* handle, const char* data, size_t size, uint32_t sections) {
