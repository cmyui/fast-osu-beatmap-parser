...
struct BeatmapConverter {
  ...
  PythonRef hit_object(const fosu::HitObject& h) {
    ...
    PythonRef start = number(h.time), end = number(h.end_time);
    const Value common[] = {{f_start_time, retain(start)},
                            {f_end_time, circle   ? retain(start)
                                         : slider ? none()
                                                  : retain(end)},
                            {f_x, integer(h.x)},
                            {f_y, integer(h.y)},
                            {f_hit_sound, sound(h.hitsound)},
                            {f_raw_type, integer(h.type)},
                            {f_raw_end_time, std::move(end)},
                            {f_is_new_combo, boolean(h.type & 4)},
                            {f_combo_skip, integer((h.type >> 4) & 7)},
                            {f_raw_hit_sample, string(h.hit_sample)}};
    if (slider) {
      const auto& s = map.sliders[h.slider];
      return record(
          t_slider,
          {{f_span_count, integer(s.slides)},
           {f_length, number(s.length)},
           {f_curve_type, curve(s.curve_type)},
           {f_raw_edge_sounds, string(s.edge_sounds)},
           {f_raw_edge_sets, string(s.edge_sets)},
  ...
  PythonRef beatmap() {
    ...
    return record(
        ...
        {{f_format_version, integer(m.format_version)},
         ...
         {f_tags, PythonRef(PyUnicode_Split(tags, nullptr, -1))},
         {f_bookmarks, bookmark_list(bookmarks)},
         {f_raw_tags, std::move(tags)},
         {f_raw_bookmarks, std::move(bookmarks)},
