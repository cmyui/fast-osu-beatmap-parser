...
bool engine_path(const char* name, char (&path)[PATH_MAX]) {
    Dl_info info{};
    if (!dladdr(reinterpret_cast<const void*>(&engine_path), &info)) return false;
    ...
    if (std::strcmp(request, "auto") == 0) {
        for (EngineKind kind : {EngineKind::Avx2, EngineKind::Neon}) {
            if (engine_available(kind))
                if (const auto* engine = load(engine_name(kind))) return engine;
        }
...
const char* engine_name(EngineKind kind) {
    switch (kind) {
        ...
        case EngineKind::Avx2: return "avx2";
        case EngineKind::Neon: return "neon";
    }
    return nullptr;
}

bool engine_available(EngineKind kind) {
    if (kind == EngineKind::Scalar) return true;
