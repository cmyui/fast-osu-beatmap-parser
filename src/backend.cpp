#include "backend.hpp"
// Separate inline definitions and arena state for each instruction set.
...
bool prepare_input(Handle& h, size_t size, const char* data) {
...
fosu_string_ref string_ref(const Handle& h, std::string_view s) {
...
int parse_owned(Handle& h, uint32_t sections) {
    char* p = h.text();
...
void destroy(void* p) { delete static_cast<Handle*>(p); }
const fosu_view* view(const void* p) {
    const auto* h = static_cast<const Handle*>(p);
    return h && h->valid ? &h->result : nullptr;
}
int parse(void* p, const char* data, size_t size, uint32_t sections) {
...
int parse_file(void* p, const char* path, uint32_t sections) {
    auto* h = static_cast<Handle*>(p);
    if (!h) return FOSU_INVALID_ARGUMENT;
    h->valid = false;
...
void cleanup() { fosu::internal::SpareArena::clear(); }
}  // namespace

#undef fosu
const fosu_dispatch::Backend& fosu_dispatch::FOSU_BACKEND_SYMBOL() {
    static constexpr Backend table = {
        FOSU_BACKEND_NAME, create, destroy, view, parse, parse_file, cleanup
    };
    return table;
}
