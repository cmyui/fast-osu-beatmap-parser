...
void* fosu_memchr(const void* s, int c, size_t n) {
  ...
  while (p < end) {
    const uint32_t m = static_cast<uint32_t>(_mm256_movemask_epi8(
        _mm256_cmpeq_epi8(_mm256_loadu_si256(reinterpret_cast<const __m256i*>(p)), v)));
    const size_t rem = static_cast<size_t>(end - p);
    const uint32_t mm = rem >= 32 ? m : (m & ((1u << rem) - 1));
    if (mm)
      return const_cast<char*>(p + _tzcnt_u32(mm));
    p += 32;
  }
  return nullptr;
}

extern "C" [[noreturn]] void __assert_fail(const char*,
                                           const char*,
                                           unsigned,
                                           const char*) {
  rt::exit(5);
