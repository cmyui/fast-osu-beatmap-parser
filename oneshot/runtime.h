...
inline long read(int fd, void* p, size_t n) {
  return sys3(SYS_read, fd, (long)p, (long)n);
}
inline long write(int fd, const void* p, size_t n) {
  return sys3(SYS_write, fd, (long)p, (long)n);
}
inline long madvise(void* p, size_t n, int adv) {
  return sys3(SYS_madvise, (long)p, (long)n, adv);
}
...
long sysconf(int name) noexcept {
  if (name == _SC_PAGESIZE)
    return 4096;  // Linux x86-64 base page size.
  errno = EINVAL;
  return -1;
}
void* mmap(void* address,
  ...
  return reinterpret_cast<void*>(
      posix_result(rt::sys6(9, (long)address, size, protection, flags, fd, offset)));
}
...
void* memmove(void* d, const void* s, size_t n) {
  if (d <= s || (const char*)d >= (const char*)s + n)
    return memcpy(d, s, n);
...
// provided by the parser TU through an asm label (see main.cc).
