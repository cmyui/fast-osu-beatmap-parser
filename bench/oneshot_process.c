...
int main(int argc, char** argv) {
  if (argc < 6) {
    fprintf(stderr, "usage: process_bench corpus limit reps bin1 bin2 ...\n");
    return 2;
  }
  ...
  while ((e = readdir(d))) {
    ...
    if (l < 4 || strcmp(e->d_name + l - 4, ".osu"))
      continue;
    if (n == cap)
      names = realloc(names, (cap *= 2) * sizeof *names);
  ...
  for (size_t i = 0; i < limit; ++i) {
    ...
    for (int r = 0; r < reps; ++r) {
      for (int j = 0; j < bins; ++j) {
        ...
        pid_t pid;
        uint64_t start = now();
        int rc = posix_spawn(&pid, child[0], &fa, NULL, child, environ);
        if (rc) {
