...
def summarize(records, metadata, cohorts=None):
    ...
    for record in records:
        name, kind, file, round_id = (
            record[k] for k in ("variant", "workload", "file", "round")
        )
        ...
        if "error" not in record:
            if len(record["ns"]) != reps or any(
                not math.isfinite(t) or t <= 0 for t in record["ns"]
            ):
    ...
    if modes and (
        set(modes) != files or any(mode not in range(4) for mode in modes.values())
    ):
    ...
    for name, kind in sorted(expected):
        ...
        for file in sorted(files):
            ...
            if any("error" in r for r in refs.values()):
                raise ValueError(
                    "Reference failed; cannot establish a comparison cohort"
                )
        ...
        coverage[f"{name}/{kind}"] = {
            ...
            "failed_files": errors,
            "errors_by_type": dict(
                Counter(error for kinds in errors.values() for error in kinds)
            ),
        }
        if modes:
    ...
    if modes:
        for group, members in list(groups.items()):
            ...
            groups[all_modes] = [
                m
                for m in members
                if set(variants[m[0]].get("modes", range(4))) == set(range(4))
            ]
            scopes[all_modes] = files
            for mode in sorted(set(modes.values())):
                name = f"{group}_mode_{mode}"
                groups[name] = [
                    m for m in members if mode in variants[m[0]].get("modes", range(4))
                ]
    ...
    for group, members in groups.items():
        ...
        for name, kind in members:
            ...
            passes = [
                [
                    t
                    for file in sorted(cohort)
                    for t in results[name, kind, file][i]["ns"]
                ]
                for i in range(rounds)
            ]
        ...
        tables[group] = {
            "files": len(cohort),
            "bytes": byte_count,
            "excluded_files": sorted(files - cohort),
            "rows": rows,
        }
        if modes:
            tables[group]["modes"] = dict(
                sorted(Counter(str(modes[f]) for f in cohort).items())
            )
...
def main():
    ...
    parser.add_argument(
        "--cohorts", type=Path, help="Keep a prior report's exact table cohorts"
    )
    ...
    report["run"]["versions"] = {
        v["name"]: v["version"] for v in metadata["config"]["variants"]
    }
    if args.cohorts:
        report["run"]["cohort_source_sha256"] = hashlib.sha256(
            args.cohorts.read_bytes()
        ).hexdigest()
    report["run"]["raw_results_sha256"] = hashlib.sha256(
    ...
    with gzip.GzipFile(filename=str(samples_path), mode="wb", mtime=0) as compressed:
        with (
            io.TextIOWrapper(compressed, newline="") as destination,
            args.results.open() as source,
        ):
    ...
    report["run"]["samples_sha256"] = hashlib.sha256(
        samples_path.read_bytes()
    ).hexdigest()
    shutil.copyfile(
        args.results.with_suffix(".corpus.csv.gz"),
        args.output.with_suffix(".corpus.csv.gz"),
    )
