...
def main():
    ...
    for round_id in range(2):
        for variant, kind in jobs if round_id == 0 else reversed(jobs):
            ...
            identity = {key: sample[key] for key in ("files", "bytes", "corpus_sha256", "counts")}
    ...
    for variant, kind in jobs:
        times = [s["elapsed_ns"] / s["files"] / 1000 for s in report["samples"]
                 if s["variant"] == variant["name"] and s["workload"] == kind]
        report["rows"][f"{variant['name']}/{kind}"] = {
            "mean_us": statistics.mean(times), "pass_mean_us": times,
        }
    report.update(complete=True, finished_utc=datetime.now(timezone.utc).isoformat(),
                  objects=sum(reference["counts"]), files=reference["files"],
                  bytes=reference["bytes"], corpus_sha256=reference["corpus_sha256"])
    args.output.write_text(json.dumps(report, indent=2) + "\n")
