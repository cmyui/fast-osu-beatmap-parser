...
$compiler $flags $ldflags -Iinclude -fPIC -fvisibility=hidden -shared bench/library_module.cpp -o "$output/baseline.so"
$compiler -std=c++20 -O2 -fstack-protector-strong -fstack-clash-protection -D_FORTIFY_SOURCE=3 $ldflags -fPIE -pie -Iinclude bench/library_compare.cpp -ldl -o "$output/compare"
$compiler $flags $ldflags -Iinclude -fPIC -fvisibility=hidden -shared -fprofile-generate="$profile" bench/library_module.cpp -o "$output/profiled.so"
FOSU_BENCH_SPLIT=train "$output/compare" "$corpus" 3 "$output/profiled.so" > "$output/training.csv"
$compiler $flags $ldflags -Iinclude -fPIC -fvisibility=hidden -shared -fprofile-use="$profile" -fprofile-correction -Werror=missing-profile bench/library_module.cpp -o "$output/profiled.so"
