...
int main(int argc, char** argv) {
  if (argc < 5) {
    fprintf(stderr, "usage: profile_parse corpus limit rounds fresh|reuse [sections]\n");
    return 2;
  }
  ...
  if (!limit || rounds < 1 || (strcmp(argv[4], "fresh") && strcmp(argv[4], "reuse")))
    return 2;
  const bool reuse = strcmp(argv[4], "reuse") == 0;
  const uint32_t sections =
      argc > 5 ? static_cast<uint32_t>(strtoul(argv[5], nullptr, 0)) : fosu::kAllSections;
  ...
  printf(
      "files %zu bytes %zu rounds %d mode %s sections 0x%x: best round %.1f us (%.3f "
