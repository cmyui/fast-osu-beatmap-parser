...
int main(int argc, char** argv) {
    if (argc < 4) { fprintf(stderr, "usage: library_compare corpus reps module1 [module2...]\n"); return 2; }
    ...
    for (int i = 3; i < argc; ++i) {
        ...
        auto parse = reinterpret_cast<void(*)(void*, const char*, size_t, int)>(dlsym(dso, "fosu_bench_parse"));
        auto free = reinterpret_cast<void(*)(void*)>(dlsym(dso, "fosu_bench_free"));
        if (!create || !parse || !free) return 1;
    ...
    if (split && strcmp(split, "train") && strcmp(split, "eval")) return 2;
    if (split) {
        std::vector<std::string> selected;
        for (size_t i = 0; i < files.size(); ++i)
            if ((i % 5 == 0) == (strcmp(split, "train") == 0)) selected.push_back(files[i]);
    ...
    for (size_t i = 0; i < files.size(); ++i) {
        if (!fosu::read_into(files[i].c_str(), input)) return 1;
        for (int r = 0; r < reps; ++r) {
            for (size_t j = 0; j < modules.size() * 2; ++j) {
                ...
                printf("%s,%zu,%d,%s,%d,%llu\n", files[i].c_str(), input.size,
                       r, m.name, reuse, static_cast<unsigned long long>(ns));
            }
        }
    }
    for (auto& m : modules) { m.free(m.ctx); dlclose(m.dso); }
}
