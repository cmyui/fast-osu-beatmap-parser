...
int main() {
  ...
  for (int i = 0; i < 4096; ++i) {
    snprintf(buf, sizeof buf, "%d,%d,%d,%d,%d,0:0:0:0:", int(rng() % 512),
             int(rng() % 384), int(rng() % 100000000), int(rng() % 4) == 0 ? 5 : 1,
             int(rng() % 16));
      ...
      for (int it = 0; it < iterations; ++it)
        for (const auto& line : lines) {
          if (simd) {
            ...
            if (prefix)
              sink += static_cast<uint32_t>(prefix->value.time);
          } else {
            const auto prefix =
                fosu::internal::parse_hitobject_prefix_scalar(line.data(), line.size());
            if (prefix)
              sink += static_cast<uint32_t>(prefix->value.time);
          }
        }
