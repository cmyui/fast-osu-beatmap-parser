...
with a.csv.open(newline='') as stream:
    for row in csv.DictReader(stream):
        if int(row.get('exit_status', 0)):
            p.error('a measured process failed')
        kind = row.get('workload', 'reuse=' + row['reuse'] if 'reuse' in row else 'process')
        groups[(row['variant'], kind)][row['file']].append(row)
if not groups:
    p.error('no measurements')
summaries = []
for (variant, kind), files in groups.items():
    ...
    result = dict(variant=variant, workload=kind, files=len(files), runs=len(all_values),
                  reps_min=min(map(len, files.values())), reps_max=max(map(len, files.values())),
                  mean_file_min_us=mean(values), p50_file_min_us=median(values),
                  p90_file_min_us=values[int(.9 * len(values))],
                  all_mean_us=mean(all_values), all_p50_us=median(all_values))
    if 'bytes' in best[0]:
