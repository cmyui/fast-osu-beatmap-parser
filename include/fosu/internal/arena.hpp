...
// holds the padded input copy and record arrays, avoiding separate array
// allocations while they fit. Reused handles can retain that mapping, and a
...
//
// Recycling retains storage; every parse replaces the previous logical result.
#include <sys/mman.h>
...
public:
    ...
    size_t size() const { return e_ == b_ ? 0 : static_cast<size_t>(e_ - b_); }
    size_t capacity() const { return c_ == b_ ? 0 : static_cast<size_t>(c_ - b_); }
    ...
    void set_size(size_t n) { e_ = n ? b_ + n : b_; }
    template <typename... A>
    T& emplace_back(A&&... a) {
...
private:
    ...
    T* allocate(size_t n, bool& on_heap) {
        if (n > static_cast<size_t>(PTRDIFF_MAX) / sizeof(T))
...
// One recycled arena per loaded library image: a released handle parks it here
...
// Register teardown when the library loads, without a local-static guard on
