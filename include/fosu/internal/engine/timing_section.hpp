...
inline const char* parse_timing_points_section(
    ...
    while (p < file_end) {
        ...
        if (length - 15 <= 64 - 15) [[likely]] {
            ...
            const uint64_t commas =
                (equal_mask32(first, comma_value) |
                 static_cast<uint64_t>(equal_mask32(second, comma_value))
                     << 32) &
                line_mask;
            const uint64_t nondigits =
                (nondigit_mask32(first, bias, threshold) |
                 static_cast<uint64_t>(
                     nondigit_mask32(second, bias, threshold))
                     << 32) &
                line_mask;
            ...
            if (tp_shape_match(row, commas, nondigits, length, p)) {
                ...
                accepted = fast_parse_timing_point_masked(
                    commas, nondigits, p, length, point, &geometry);
                if (accepted)
                    tp_shape_insert(
                        cache, commas, nondigits, length, geometry);
            }
        }
        ...
        if (!ignored_line(p, line_end)) {
            if (parse_timing_fields(p, line_end, point))
                beatmap.timing_points[point_count++] = point;
            else
                ++malformed;
        }
