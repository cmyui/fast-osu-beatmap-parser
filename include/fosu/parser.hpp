...
#include "internal/engine/parse_document.hpp"
...
inline bool allocate_beatmap_arrays(
    Arena* arena, Beatmap& beatmap,
...
public:
    ...
    Result<Beatmap*> parse(
        const char* data, size_t size, ParseOptions opts = {}) noexcept {
    ...
    Result<Beatmap*> parse(
        std::span<const char> input, ParseOptions opts = {}) noexcept {
        return parse(input.data(), input.size(), opts);
    }

    Result<Beatmap*> parse(
        const FileBuffer& input, ParseOptions opts = {}) noexcept {
        return parse(input.data.get(), input.size, opts);
    }

    Result<Beatmap*> parse_file(
        const char* path, ParseOptions opts = {}) noexcept {
        reset_working_result();
        ...
        while (bytes_read < size) {
            ...
            if (count <= 0) {
                ...
                reset_working_result();
                return Error{
                    ErrorCode::IoFailure, kNoErrorOffset, error};
            }
...
private:
    ...
    Result<char*> prepare_input(size_t size, const char* data) noexcept {
        if (size > kMaxInputSize)
            return Error{ErrorCode::InputTooLarge};
        if (!arena_) arena_ = internal::acquire_parser_arena();
        if (!arena_) return Error{ErrorCode::AllocationFailure};
        input_ = static_cast<char*>(arena_push(arena_, size + kInputExtra, 1));
        if (!input_) return Error{ErrorCode::AllocationFailure};
    ...
    Result<Beatmap*> finish_parse(ParseOptions opts) noexcept {
        if (input_size_ != 0) {
            const auto sizes = internal::maximum_beatmap_array_sizes(
                input_size_, opts.sections);
